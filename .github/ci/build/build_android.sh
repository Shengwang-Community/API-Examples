...
if [ "$api_examples_shengwang_branch_stripped" = "main" ]; then
    ...
    BRANCH_NAME_VERSION=$(echo $api_examples_shengwang_branch | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -n 1)
...
if [ "$compress_apiexample" = true ]; then
    ...
    OUTPUT_ZIP_NAME="Shengwang_Native_SDK_for_Android_v${SDK_VERSION}_${android_direction}_${BUILD_NUMBER}.zip"
    ...
    OUTPUT_ZIP_NAME="withSDK_Shengwang_Native_SDK_for_Android_v${SDK_VERSION}_${android_direction}_${BUILD_NUMBER}.zip"
    echo "Output zip name: $OUTPUT_ZIP_NAME"
    mv $TEMP_ZIP_NAME $WORKSPACE/$OUTPUT_ZIP_NAME
