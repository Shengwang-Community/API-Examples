...
# Args:
#   $1 - Branch name
# Returns: Version string (e.g., "4.6.2") or empty if not in <type>/x.x.x format
...
validate_sdk_version() {
    ...
    # Support both Agora and Shengwang package names
    local sdk_version=""
    if [ "$sdk_type" = "macos" ]; then
        # Try Agora first, then Shengwang
        sdk_version=$(grep -E "^[[:space:]]*#?[[:space:]]*pod[[:space:]]+'AgoraRtcEngine_macOS'" "$podfile_path" | sed -n "s/.*'\([0-9.]*\)'.*/\1/p" | head -1)
        if [ -z "$sdk_version" ]; then
            sdk_version=$(grep -E "^[[:space:]]*#?[[:space:]]*pod[[:space:]]+'ShengwangRtcEngine_macOS'" "$podfile_path" | sed -n "s/.*'\([0-9.]*\)'.*/\1/p" | head -1)
        fi
    else
        # iOS: support both AgoraRtcEngine_iOS and AgoraAudio_iOS, and Shengwang variants
        ...
        if [ -z "$sdk_version" ]; then
            sdk_version=$(grep -E "^[[:space:]]*#?[[:space:]]*pod[[:space:]]+'ShengwangRtcEngine_iOS'" "$podfile_path" | sed -n "s/.*'\([0-9.]*\)'.*/\1/p" | head -1)
        fi
        if [ -z "$sdk_version" ]; then
            sdk_version=$(grep -E "^[[:space:]]*#?[[:space:]]*pod[[:space:]]+'ShengwangAudio_iOS'" "$podfile_path" | sed -n "s/.*'\([0-9.]*\)'.*/\1/p" | head -1)
        fi
    fi
...
run_version_validation() {
    ...
    # Extract version from branch name (format: <type>/x.x.x)
    ...
    if [ -z "$BRANCH_VERSION" ]; then
        ...
        echo "=========================================="
        echo "Current branch: $BRANCH_NAME"
        echo "Required format: <type>/x.x.x (e.g., dev/4.5.3, release/4.5.3)"
        echo ""
        echo "Branch naming rules:"
        echo "  - Version branches must follow: <type>/x.x.x"
