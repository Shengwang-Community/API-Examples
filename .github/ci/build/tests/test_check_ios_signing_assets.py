...
class SigningAssetCheckTests(unittest.TestCase):
    ...
    def test_distribution_certificate_names_are_interchangeable(self):
        aliases = (
            "iPhone Distribution",
            "Apple Distribution",
            "iOS Distribution",
        ...
        for selector in aliases:
            for identity_prefix in aliases:
                with self.subTest(selector=selector, identity=identity_prefix):
                    self.assertTrue(
                        MODULE.certificate_selector_matches(
                            selector,
                            f"{identity_prefix}: Example (TEAM123)",
                        )
                    )
