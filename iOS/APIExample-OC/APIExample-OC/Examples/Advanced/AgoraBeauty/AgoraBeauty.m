...
@interface AgoraBeautyEntry ()
@property (weak, nonatomic) IBOutlet UITextField *textField;

@end

@implementation AgoraBeautyEntry
...
- (IBAction)onClickJoinButton:(id)sender {
    ...
    UIStoryboard *storyBoard = [UIStoryboard storyboardWithName:@"AgoraBeauty" bundle:nil];
    BaseViewController *newViewController = [storyBoard instantiateViewControllerWithIdentifier:@"AgoraBeauty"];
...
@interface AgoraBeauty ()<AgoraRtcEngineDelegate>
...
@property (nonatomic, strong) AgoraBeautyManager *beautyManager;

@end

@implementation AgoraBeauty
...
- (void)viewDidLoad {
    ...
    self.beautyManager = [[AgoraBeautyManager alloc] initWithAgoraKit:self.agoraKit];
}

- (IBAction)onChangeBeauty:(UISwitch *)sender {
    [self.beautyManager setBasicBeautyEnable:sender.isOn];
}
- (IBAction)onLightenSlider:(UISlider *)sender {
    self.beautifyOption.lighteningLevel = sender.value;
    [self.beautyManager setLightness:sender.value];
}
- (IBAction)onRednessSlider:(UISlider *)sender {
    self.beautifyOption.rednessLevel = sender.value;
    [self.beautyManager setRedness:sender.value];
}
- (IBAction)onSharpnessSlider:(UISlider *)sender {
    self.beautifyOption.sharpnessLevel = sender.value;
    [self.beautyManager setSharpness:sender.value];
}
- (IBAction)onSmoothSlider:(UISlider *)sender {
    self.beautifyOption.smoothnessLevel = sender.value;
    [self.beautyManager setSmoothness:sender.value];
}
- (IBAction)onChangeLowLightEnhance:(UISwitch *)sender {
    [self.beautyManager setLowLightEnhance:sender.isOn];
}
- (IBAction)onChangeVideoDenoise:(UISwitch *)sender {
    [self.beautyManager setVideoDenoise:sender.isOn];
}
- (IBAction)onChangeColorEnhance:(UISwitch *)sender {
    [self.beautyManager setColorEnhance:sender.isOn strengthValue:self.strength skinProtectValue:self.skinProtect];
}
- (IBAction)onStrengthSlider:(UISlider *)sender {
    self.strength = sender.value;
    [self.beautyManager setColorEnhance:self.colorEnhanceSwitch.isOn strengthValue:self.strength skinProtectValue:self.skinProtect];
}
