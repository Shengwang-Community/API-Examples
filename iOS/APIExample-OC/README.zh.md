...
这个开源示例项目演示了声网 RTC SDK 的部分 Objective-C API，帮助开发者理解和使用声网 RTC SDK。
...
运行后确认 `APIExample-OC.xcworkspace` 正常生成即可。
...
5. 打开 `APIExample-OC.xcworkspace` 并编辑 `KeyCenter.m`，将你的 AppID 和 Certificate 分别替换到 `<#Your APPID#>` 与 `<#YOUR Certificate#>`
...
然后你就可以使用 `APIExample-OC.xcworkspace` 编译并运行项目了。
