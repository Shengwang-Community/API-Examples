...
这个开源示例项目演示了声网 RTC SDK 的部分 macOS API，帮助开发者理解和使用声网 RTC SDK。
...
进入 `macOS` 目录并安装 CocoaPods 依赖。CocoaPods 的安装方式请参考[官方指南](https://guides.cocoapods.org/using/getting-started.html)。
...
声网美颜 2.0 资源包不包含在仓库中。本地构建前，从声网技术支持获取 `AgoraBeautyMaterial.bundle.zip`，并解压到 `macOS/APIExample/Resources/`。
