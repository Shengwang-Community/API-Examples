...
Use a nearby dialog with the same media path as the implementation reference, then adapt
it to the new behavior contract and current SDK APIs.

### Step 4: Create Dialog Implementation File

Create `APIExample/APIExample/[Basic|Advanced]/<ExampleName>/C<ExampleName>Dlg.cpp`

Keep the implementation paired with the header and follow the selected neighboring
dialog's message-map and lifecycle structure. Provide public `InitAgora()` and
`UnInitAgora()` for host scene activation/exit. `OnInitDialog()` initializes UI only because
the host precreates these windows. Cleanup must be idempotent, invalidate pending work,
leave the channel, stop case-owned media and release the engine before the next scene
starts. Keep the event receiver alive through release and discard old queued messages.
The current SDK's `release(nullptr)` is synchronous; never release in an SDK callback.
...
- **SDK documentation:** Refer to Shengwang RTC SDK for Windows documentation for API details
